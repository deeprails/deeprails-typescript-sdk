...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['DEEPRAILS_LOG'] = undefined;
    });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['DEEPRAILS_LOG'] = 'debug';
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['DEEPRAILS_LOG'] = 'not a log level';
      const client = new Deeprails({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'DEEPRAILS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['DEEPRAILS_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['DEEPRAILS_BASE_URL'] = 'https://example.com/from_env';
      const client = new Deeprails({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['DEEPRAILS_BASE_URL'] = ''; // empty
      const client = new Deeprails({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.deeprails.com');
    });

    test('blank env variable', () => {
      process.env['DEEPRAILS_BASE_URL'] = '  '; // blank
      const client = new Deeprails({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.deeprails.com');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['DEEPRAILS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Deeprails({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
